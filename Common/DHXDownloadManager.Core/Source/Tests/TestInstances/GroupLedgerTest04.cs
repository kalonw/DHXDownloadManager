...
namespace DHXDownloadManager.Tests
{
    ...
    class GroupLedgerTest04<T> : Test<T> where T : IDownloadEngine, new()
    {

        public GroupLedgerTest04(Tests<T> parent) : base(parent) { }
        int downloadCount = 0;
        override protected IEnumerator _DoTest()
        {
            ...
            if (group.Status == Group.StatusFlags.Destroyed && manifestDestroyed)
                Success();
            else
                Fail();
