...
namespace DHXDownloadManager.Tests
{
    ...
    class GroupLedgerTest01<T> : Test<T> where T : IDownloadEngine, new()
    {
        ...
        override protected IEnumerator _DoTest()
        {
            ...
            metadata1 = new ManifestFileStream("https://s3.amazonaws.com/piko_public/Test.png", System.IO.Path.GetTempPath(), ManifestFileStream.Flags.None);
            metadata2 = new ManifestFileStream("https://s3.amazonaws.com/piko_public/Test2.png", System.IO.Path.GetTempPath(), ManifestFileStream.Flags.None);
            metadata3 = new ManifestFileStream("https://s3.amazonaws.com/piko_public/Test3.png", System.IO.Path.GetTempPath(), ManifestFileStream.Flags.None);
            metadata3 = new ManifestFileStream("httpsf://s3.amazonaws.com/piko_public/Test4.png", System.IO.Path.GetTempPath(), ManifestFileStream.Flags.None);
        ...
        {
            downloadCount--;
            Logger.Log("group_OnDownloadGroupEndDownload " + obj.Key + " " + downloadCount);
        }

        void group_OnDownloadGroupStartDownload(Group obj)
        {
            downloadCount++;
            Logger.Log("group_OnDownloadGroupStartDownload " + obj.Key + " " + downloadCount);
        }
